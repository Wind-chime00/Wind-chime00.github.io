    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Found you!˃ʍ˂",
            message: "Remember to follow the GPL license!",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "warning",
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://github.com/Wind-chime00/web-information/blob/bc8beea5638e441fd872b6dea0c4067f6c38240d/b_1f298c039cff93c86cd97cf100f27542.jpg",
        hoverMsg: "Return to the top~",
...
function createtime1() {
  var grt = new Date("17/03/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
  now1.setTime(now1.getTime() + 250);
  var days = (now1 - grt) / 1000 / 60 / 60 / 24;
  var dnum = Math.floor(days);

  var ascll = [
    `Welcome to Junhe's personal web!`,
    //`Future is now 🍭🍭🍭`,
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Fomalhaut🥝 %c 你正在访问Junhe's personal web!",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Junhe's") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Junhe share\n标题：' + subTitle + '\n链接：' + url + '\nWelcome！');
    new Vue({
      data: function () {
        this.$notify({
          title: "Share information copied successfully.",
          message: "You can now paste and share it directly with your friends!",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime() {
  ...
  var start = new Date("2025-03-17T00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("2025-03-17T00:00:00");
...
if (window.localStorage.getItem("fpson") == undefined || window.localStorage.getItem("fpson") == "1") {
  ...
  var loop = function () {
    ...
    if (now > 1000 + lastTime) {
      ...
      if (fps <= 5) {
        var kd = `<span style="color:#bd0000">What are u doing?</span>`
      } else if (fps <= 15) {
        var kd = `<span style="color:red">Not good</span>`
      } else if (fps <= 25) {
        var kd = `<span style="color:orange">Feeling bad.</span>`
      } else if (fps < 35) {
        var kd = `<span style="color:#9338e6">Not quite fluent</span>`
      } else if (fps <= 45) {
        var kd = `<span style="color:#08b7e4">Not bad at all!</span>`
      } else {
        var kd = `<span style="color:#39c5bb">Pretty good!</span>`
      }
